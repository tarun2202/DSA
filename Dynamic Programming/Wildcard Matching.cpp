...
public:
    ...
    /*bool wildCardMatching(string &s, int i, string &p, int j, int lastIndexOfAllStar, 
        ...
        // when s gets exhausted: return true if s contain only *, otherwise false.
        if(i == -1)
        ...
        if(p[j]=='*'){
            bool take= wildCardMatching(s, i, p, j-1, lastIndexOfAllStar, dp);
            bool notTake = wildCardMatching(s, i-1, p, j, lastIndexOfAllStar, dp);
