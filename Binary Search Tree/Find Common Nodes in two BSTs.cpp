...
//naive approach
Time Complexity : O( NlogN + M ), N is the number of elements in first tree and M is number of elements in second tree.
Space Complexity : O(N), extra space is used to store elements in set.

optimised approach
Time Complexity: O(N+M), Here ‘m’ and ‘n’ are number of nodes in first and second tree respectively,as we need to traverse both the trees.
Space Complexity: O(H1+H2), Use of stack for storing values, at-most elements = ‘Height of tree'
*/
